...
export function part2(): void {
  ...
  const currentSize = dict["/"].getDirSize(dict);
  const freeSpace = totalDiskSpace - currentSize;
  const deleteSize = spaceNeeded - freeSpace;

  const dirs = [];
  for (const key in dict) {
    const dirSize = dict[key].getDirSize(dict);
    if (dirSize >= deleteSize) {
      dirs.push(dirSize);
    }
  }
  console.log(dirs.sort((a, b) => a - b)[0]);
}
